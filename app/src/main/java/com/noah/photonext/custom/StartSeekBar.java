...
/**
 * Created by HuyLV-CT on 05-Jan-17.
 */

public class StartSeekBar extends View {
    ...
    public interface OnSeekBarChangeListener {
        void onSeekBarValueChange(StartSeekBar bar,
                                  int value);
        void onStartTrackingTouch(StartSeekBar bar);
        void onStopTrackingTouch(StartSeekBar bar,int value);
    }
    ...
    public StartSeekBar(Context context) {
        this(context, null);
    }

    public StartSeekBar(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public StartSeekBar(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        // Attribute initialization
        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.StartSeekBar,
                defStyleAttr, 0);

        Drawable thumbImageDrawable = a.getDrawable(R.styleable.StartSeekBar_thumbDrawable);
        if (thumbImageDrawable == null) {
            thumbImageDrawable = getResources().getDrawable(R.mipmap.thumb_normal);
        }
        this.thumbImage = ((BitmapDrawable) thumbImageDrawable).getBitmap();

        Drawable thumbImagePressedDrawable = a.getDrawable(R.styleable.StartSeekBar_thumbPressedDrawable);
        if (thumbImagePressedDrawable == null) {
            thumbImagePressedDrawable = getResources().getDrawable(R.mipmap.thumb_pressed);
        }
        this.thumbPressedImage = ((BitmapDrawable) thumbImagePressedDrawable).getBitmap();

        this.absoluteMinValue = a.getFloat(R.styleable.StartSeekBar_minValue, DEFAULT_MIN_VALUE);
        this.absoluteMaxValue = a.getFloat(R.styleable.StartSeekBar_maxValue, DEFAULT_MAX_VALUE);
        this.startValue = a.getFloat(R.styleable.StartSeekBar_startValue,0);
        setProgress(startValue);
        currentValue = startValue;

        this.defaultBackgroundColor = a.getColor(R.styleable.StartSeekBar_defaultBackgroundColor,
                DEFAULT_BACKGROUND_COLOR);
        this.defaultRangeColor = a.getColor(R.styleable.StartSeekBar_defaultBackgroundRangeColor,
                DEFAULT_RANGE_COLOR);
            ...
            case MotionEvent.ACTION_MOVE:
                if (isThumbPressed) {
                    ...
                    if (notifyWhileDragging && listener != null) {
                        currentValue = normalizedToValue(normalizedThumbValue);
                        listener.onSeekBarValueChange(this, (int)currentValue);
                    }
                }
                break;
    ...
    void onStopTrackingTouch() {
        listener.onStopTrackingTouch(this, (int) normalizedToValue(normalizedThumbValue));
        isDragging = false;
    }
